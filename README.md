...
unshift()를 사용해 배열의 앞에서부터 값이 들어가게 했다.<br/>
...
_2021 02 10_<br/>
**자리수 더하기**<br/>
자연수 n이 주어졌을 때 각 자리의 숫자를 더한 값을 구하는 문제<br/>
주어진 수를 배열로 바꾸어 각자리의 합을 리턴해주었다.<br/><br/><br/>

_2021 02 12_<br/>
**자연수 뒤집어 배열로 만들기**<br/>
우선 n을 String()을 이용해 문자형으로 바꿔주고 split('')을 이용해 <br/>
각 자리를 나눠 배열에 넣었다. reverse()를 이용해 배열을 뒤집어주고, <br/>
for문을 통해 각 요소를 Number에 넣어 숫자형으로 바꿔주었다.<br/><br/><br/>

_2021 02 12_<br/>
**정수 내림차순으로 배치하기**<br/>
이전 문제와 비슷하게 문자열로 바꾸어 배열로 넣어주고 sort()를 이용해<br/>
내림차순 정렬한 join()해주고 숫자형으로 리턴해주었다.<br/><br/><br/>

_2021 02 12_<br/>
**정수 제곱근 판별**<br/>
주어진 n이 어떤 정수의 제곱으로 만들어 지는지 확인하는 문제<br/>
for문을 통해 i\*i가 n인지 확인해보았다.<br/>
for문의 조건을 보다 잘 준다면 더 효율적인 코드를 만들 수 있을듯 하다.<br/>
물론 바빌로니아 법이라는 제곱근을 구하는 알고리즘이 이미 있기에<br/>
내장함수 Math.sqrt()를 사용한다면 보다 빠르게 구할 수 있다.<br/><br/><br/>

_2021 02 12_<br/>
**제일 작은 수 제거하기**<br/>
정수로 채워진 배열에서 가장 작은 요소를 제거한 배열을 리턴하는 문제<br/>
가장 작은 요소를 찾기위해 우선 배열을 내림차순하여 요소를 구했다.<br/>
해당 요소의 인덱스 번호에 맞게 splice()시켜 해결!<br/>
알아보니 Math.min이라는 내장함수를 이용해 가장 작은 요소를 찾을 수 있었다.<br/><br/><br/>

_2021 02 12_<br/>
**짝수와 홀수**<br/>
2로 나누어 떨어지는 지 확인해 리턴해주었다.<br/><br/><br/>

_2021 02 19_<br/>
**다리를 지나는 트럭**<br/>
큐를 이용하면 쉽게 풀 수 있는 문제<br/>
처음에는 큐를 쓰지 않고 풀어보려고 용을 썼지만 고려할 것도 많고<br/>
시간을 구하려고 하면 트럭이 지나가는 것을 놓치고 트럭을 잘 보내면<br/>
중첩되는 시간을 놓쳐서 풀리지 않았다. 결국 큐를 이용해서 푸는데<br/>
이게 어떤 로직을 짜느냐에 따라 시간 차이가 많이 벌어졌다.<br/>
처음에는 모든걸 반복문을 통해 돌렸는데 시간이 3000ms가 나왔다.<br/>
두번째는 반복을 최소화 하기 위해 값을 더하고 빼주는 식으로 로직을 짜자<br/>
시간이 반으로 줄었다. 마지막으로 기존에 사용하던 splice함수는 배열을 모두 이동시킨다하여<br/>
shift함수로 바꾸어 작성하자 시간이 28ms로 거의 50배 차이가 났다...<br/>
앞으로 코드를 짤 때 더 효율적인 로직을 짜고 효율적인 함수를 사용하도록 신경써야겠다.<br/><br/><br/>

_2021 02 22_<br/>
**124 나라의 숫자**<br/>
124로 숫자를 표현하는 이상한 나라의 숫자 표기법으로 숫자를 바꾸는 문제<br/>
3진법이랑 비슷하지만 이상하게 숫자가 움직여서 애를 먹었다.<br/>
침착하게 조건에 맞게 하나씩 함수를 써 내려가서 풀긴 풀었지만<br/>
솔직히 만족스런운 풀이는 아니었다... 다른 사람들은 한 줄만에 풀기도 하는데<br/>
나는 아직 많이 멀은 것 같다..<br/><br/><br/>

_2021 02 22_<br/>
**스킬 트리**<br/>
주어진 문자열의 순서에 어긋나는 걸 제외하고 남은 수를 리턴하는 문제<br/>
문제 자체는 어렵지 않았는데 배열과 문자열로 주어진게 조금 성가겼다.<br/>
무엇보다 순서에 맞는데 뒤에 오는 문자가 아예 존재하지 않는 경우를 골라주는게<br/>
조금 까다로웠다. 하지만 침착하게 조건을 부여하니 통과!<br/><br/><br/>

_2021 02 22_<br/>
**기능 개발**<br/>
두 배열을 서로 더해가면서 앞에서 부터 조건에 충족된 애들을 한번씩 가져오는 문제<br/>
처음에는 복잡해서 당황했지만 역시나 차근차근 조건에 맞게 코드를 짜니 문제가 풀렸다.<br/>
실제로 코드를 돌려보니 아주 느린 코드도 아니었고..<br/>
레벨2에 올라면서 문제에 접근법이 어려워진것 말고는 도전해봄직한 것 같다.<br/><br/><br/>

_2021 02 22_<br/>
**프린터**<br/>
스택/큐 문제. 이미 트럭 문제에서 스택/큐의 필요성을 절실히 느껴<br/>
이상한 방법을 쓰지 않고 바로 배열을 이용해 문제를 풀어갔다.<br/>
조건을 충족시키기가 까다로워 조금 돌아갔지만 그래도 침착하게 성공<br/><br/><br/>

_2021 02 28_<br/>
**문자열 압축**<br/>
문자열을 압축시켜 문자열을 줄이는 문제.<br/>
조건은 항상 압축 길이가 첫번째 문자를 포함하고 시작해야 한다는 것이다.<br/>
우선 작은 단위로 압축할 경우와 큰 단위로 압축할 경우 모두 최소값이 나올 수 있기에<br/>
for문을 통해 모든 경우를 검사해볼 수 밖에 없었다. 그래도 시간을 단축하기 위해<br/>
주어진 문자열의 길이의 반까지만 나누는 단위 값을 증가시켜 주었다.<br/>
for문을 돌면서 똑같은 문자열이 등장하면 그 문자열의 카운트값과 해당 문자열을 다른 배열에<br/>
push해주고 마지막에 그 배열의 길이와 지금까지의 최솟값을 비교해주었다.<br/><br/><br/>
